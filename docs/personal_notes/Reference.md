# N-Body Simulation References
- [ChaNGa parallel N-Body simulator user guide](https://github.com/N-BodyShop/changa/wiki/ChaNGa-User-Guide)
- [BEHALF - Barnes Hut Algorithm](https://anaroxanapop.github.io/behalf/)
- [The Barnes-Hut Algorithm Tom Ventimiglia & Kevin Wayne](https://arborjs.org/docs/barnes-hut)
- [Barnes-Hut Galaxy Simulator](https://www.cs.princeton.edu/courses/archive/fall03/cs126/assignments/barnes-hut.html)
- [Barnes-Hut - Data Structure and Benchmark](https://iss.oden.utexas.edu/?p=projects/galois/scientific/barnes_hut)
- [MPI Barnes-Hut](https://www.cs.utexas.edu/~rossbach/cs380p/lab/bh-submission-cs380p.html)
- [Fast N-Body Simulation with CUDA](https://developer.nvidia.com/gpugems/gpugems3/part-v-physics-simulation/chapter-31-fast-n-body-simulation-cuda)
- [Fast Hierarchical Methods for the N-body Problem, Part 1](https://people.eecs.berkeley.edu/~demmel/cs267/lecture26/lecture26.html)

# MPI References
- [MPI IO](https://www.cscs.ch/fileadmin/user_upload/contents_publications/tutorials/fast_parallel_IO/MPI-IO_NS.pdf)
- [Parallel Sort](https://www.geeksforgeeks.org/dsa/implementation-of-quick-sort-using-mpi-omp-and-posix-thread/)


# Advanced implementation techniques
### Low level instructions for high performance
- [PDEP — Parallel Bits Deposit](https://www.felixcloutier.com/x86/pdep)
- [Addressing architectural differences in your macOS code](https://developer.apple.com/documentation/apple-silicon/addressing-architectural-differences-in-your-macos-code)
- [Double-precision floating-point vectors](https://developer.apple.com/documentation/accelerate/double-precision-floating-point-vectors)
- [Advanced Vector Extensions (AVX)](https://www.cs.uaf.edu/courses/cs441/notes/avx/index.html)

# PBS submissions
[Running interactive jobs](https://docs.hpc.ugent.be/Linux/running_interactive_jobs/)